...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring, Writer& output )
{
  ...
  if ( !data.empty() ) {
    if ( str_len < first_unassembled_index
         || first_index >= first_unacceptable_index ) { // the data not store in availble_capacity
      data = "";
    } else {
      str_begin = first_index;
      str_end = str_len - 1;
      if ( first_index < first_unassembled_index )
        str_begin = first_unassembled_index; // throw head
      if ( str_len > first_unacceptable_index )
        str_end = first_unacceptable_index - 1; // throw tai;
